...
/// slides as a single panel. The surrogate is created at the full monitor
/// rect (`viewport`) and never repositioned; all per-frame animation is
/// expressed via `rcSource`/`rcDestination` in
/// `DwmUpdateThumbnailProperties`, avoiding any per-frame `SetWindowPos`
/// calls. The surrogate is hidden (via `SW_HIDE`) when the visible area is
/// empty.
pub struct WorkspaceSurrogate {
  ...
  ///
  /// The surrogate is created at `viewport` and stays there for the
  /// entire animation. `rcDestination` coordinates are expressed
  /// relative to `viewport`'s top-left corner in every per-frame
  /// thumbnail update.
  viewport: Rect,
...
impl WorkspaceSurrogate {
  ...
  ///
  /// `viewport` is the monitor rect; the surrogate window is fixed there
  /// for the entire animation. `rect` is the source window's screen
  /// rect, used as the thumbnail registration dimensions and the
  /// reference for per-frame coordinate math.
  ///
  ...
  pub fn new(
    ...
    let inner = NativeSurrogate::create(
      ...
      false,
      RECT::default(),
      // Workspace surrogates span the full viewport and must not be
      // rounded.
      &CornerStyle::Square,
  ...
  /// (0.0 → 1.0).
  ///
  /// The visible strip is clipped to `[monitor_x, monitor_x +
  /// monitor_width]` via `rcSource`/`rcDestination`; the surrogate
  /// window itself does not move. `slide_distance` is the effective
  /// travel distance (may be less than `monitor_width` to close the seam
  /// gap between the two workspace panels).
  ...
  /// zoom_factor` to `1.0`. `slide_distance` controls horizontal travel
  /// (see [`update_slide_horizontal`]).
  ///
  /// [`update_slide_horizontal`]: WorkspaceSurrogate::update_slide_horizontal
  ...
  /// from its own center. `rcDestination` grows from a zero-size rect at
  /// the surrogate center to the full surrogate rect, scaling the source
  /// content via DWM. Opacity is lerped according to `opacity_endpoint`.
  ...
  /// combined with a slide offset on the primary axis (`is_vertical`
  /// selects which). The surrogate is pinned at `self.viewport` (the
  /// monitor rect) for the entire animation; zoom and slide are
  /// expressed entirely via `rcSource`/`rcDestination` in
  ...
  fn slide_zoom_axis(
    ...
    // pinned at `self.viewport` (monitor top-left) for the entire
    // animation.
    let rc_dst = RECT {
  ...
  /// entire animation. The visible strip of source content is mapped to
  /// the corresponding screen area via `rcSource`/`rcDestination`, with
  /// no `SetWindowPos` calls per frame.
  ///
  /// [`update_slide_horizontal`]: WorkspaceSurrogate::update_slide_horizontal
  ...
  fn slide_axis(
    ...
    // surrogate's top-left (which equals `self.viewport`, the monitor
    // rect).
